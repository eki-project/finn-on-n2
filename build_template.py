...
if args.resume == "": 
    ...
    model_file = os.path.join(DEFAULT_OUT_DIR, "intermediate_models", args.resume + ".onnx")
    if not os.path.isfile(model_file):
        print(f"ERROR: Cannot resume from step {args.resume} because corresponding model file could not be found at {model_file}")
        sys.exit()
    found = False
    actual_steps = []
    for step in DEFAULT_STEPS:
        step_name = step if type(step) == str else step.__name__
        if step_name == args.resume:
            found = True
        if found:
